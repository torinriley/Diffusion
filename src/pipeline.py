...
def generate(
    ...
    with torch.no_grad():
        ...
        generator = initialize_generator(seed, device)
        context = encode_prompt(prompt, uncond_prompt, do_cfg, tokenizer, models["clip"], device)
        latents = initialize_latents(input_image, strength, generator, models, device, sampler_name, n_inference_steps)
        images = run_diffusion(latents, context, do_cfg, cfg_scale, models, device, sampler_name, n_inference_steps, generator)
...
def initialize_latents(input_image, strength, generator, models, device, sampler_name, n_inference_steps):
    ...
    if input_image:
        ...
        encoder.to(device)
        input_image_tensor = preprocess_image(input_image)
